...
def p_content_owner_ad_revenue_raw_a1(file_name, table_id, dataset_id):
    ...
    for i_row in range(len(data)):
        ...
        # _asset_title = data['asset_title'][i_row]
        # _asset_labels = data['asset_labels'][i_row]
        # _asset_channel_id = data['asset_channel_id'][i_row]
        # _asset_type = data['asset_type'][i_row]
        ...
        #,str(_asset_id),str(_asset_title),str(_asset_labels),str(_asset_channel_id),str(_asset_type),str(_custom_id)
        if i_row != 0 and i_row % 1500 == 0:
            errors = client.insert_rows(table, _arr)
            assert errors == []
            print(round((float(int(i_row) / float(len(data))) * 100), 2), "%" ,"------- %s seconds -------" % round((time.time() - start_time),2))
            time.sleep(5)
            _arr = []
def p_content_owner_video_metadata_a2(file_name, table_id, dataset_id,date):
    data = pd.read_table(file_name, sep=",")
    data = data.replace(np.nan, '', regex=True)
    table_ref = client.dataset(dataset_id).table(table_id)
    table = client.get_table(table_ref)
            ...
            if content_owner == "yt_music":
                for table in yt_music:
                    if "owner_ad_revenue_raw" not in str(table):
                        # TODO: DAILY REPORTS
                        lastest_date_BQ  = get_latest_date_func(content_owner="check_2_projects",table=table)
                        print("--------------------------"+table+"--------------------------")
                        ...
                        if bool(_retrieve_reports) == True:
                            ...
                            for date_retreive,value_retrieve in _retrieve_reports.items():
                                ...
                                download_report(youtube_reporting, report_url, content_owner+"_"+table+"_"+str(date_retreive).split(" ")[0]+".txt")
                                for _file in check_files(FOLDER_PATH):
                                    if "p_content_owner_basic_a3" in _file and _file == content_owner+"_"+table+"_"+str(date_retreive).split(" ")[0]+".txt":
                                        p_content_owner_basic_a3(_file,table,content_owner)
                                        print("{0} Inserted successfully".format(table))
                                    elif "p_content_owner_estimated_revenue_a1" in _file and _file == content_owner+"_"+table+"_"+str(date_retreive).split(" ")[0]+".txt":
                                        p_content_owner_estimated_revenue_a1(_file,table,content_owner)
                                        print("{0} Inserted successfully".format(table))
                                    elif "p_content_owner_video_metadata_a2" in _file and _file == content_owner+"_"+table+"_"+str(date_retreive).split(" ")[0]+".txt":
                                        p_content_owner_video_metadata_a2(_file,table,content_owner)
                                        print("{0} Inserted successfully".format(table))
                                    else:
                                        pass
                            print("---------------------" + content_owner + " - " +table + " DONE! ---------------------")
                        else:
                            print("-----"+content_owner + "-" +table+" is up to date!")
                    else:
                        # TODO: MONTHLY REPORTS
                            print("----------------- Checking Monthly Report --------------------")
                            newest_month_BG  = get_lastest_month_func(content_owner="check_2_projects",table=table)
                            print("--------------------------"+table+"--------------------------")
                            _retrieve_reports_monthly = retrieve_reports(youtube_reporting,lastest_date_BQ=newest_month_BG,jobId=yt_music[str(table)],onBehalfOfContentOwner=content_owners[str(content_owner)])
                            if bool(_retrieve_reports_monthly) == True:
                                print("----- The newest_date_on_BQ: {0} but {1} on API ".format(newest_month_BG,max(_retrieve_reports_monthly.keys())))
                                ...
                                for date_retreive,value_retrieve in _retrieve_reports_monthly.items():
                                    ...
                                    download_report(youtube_reporting, report_url, content_owner+"_"+table+"_"+str(date_retreive).split(" ")[0]+".txt")
                                    for _file in check_files(FOLDER_PATH):
                                        if "p_content_owner_ad_revenue_raw_a1" in _file and _file == content_owner + "_" + table + "_" + \
                                                str(date_retreive).split(" ")[0] + ".txt":
                                            p_content_owner_ad_revenue_raw_a1(_file, table, content_owner)
                                            print("{0} Inserted successfully".format(table))
                                        else:
                                            print("Does not match any table in raw!")
                                print("---------------------" + content_owner + " - " +table + " DONE! ---------------------")
                            else:
                                print("-----" + content_owner + " - " + table + " is up to date!")
                ...
                for table in yt_kids:
                    if "owner_ad_revenue_raw" not in str(table):
                        ...
                        # TODO: MONTHLY REPORTS
                            ...
                            newest_month_BG  = get_lastest_month_func(content_owner="check_2_projects",table=table)
                            print("--------------------------"+table+"--------------------------")
                            _retrieve_reports_monthly = retrieve_reports(youtube_reporting,lastest_date_BQ=newest_month_BG,jobId=yt_kids[str(table)],onBehalfOfContentOwner=content_owners[str(content_owner)])
                            if bool(_retrieve_reports_monthly) == True:
                                print("----- The newest_date_on_BQ: {0} but {1} on API ".format(newest_month_BG,max(_retrieve_reports_monthly.keys())))
                                print("---------- ",len(_retrieve_reports_monthly)," reports need to retrieve ----------")
                                for date_retreive,value_retrieve in _retrieve_reports_monthly.items():
                ...
                for table in yt_entertainment:
                    if "owner_ad_revenue_raw" not in str(table):
                        ...
                        # TODO: MONTHLY REPORTS
                            ...
                            newest_month_BG  = get_lastest_month_func(content_owner="check_2_projects",table=table)
                            print("--------------------------"+table+"--------------------------")
                            _retrieve_reports_monthly = retrieve_reports(youtube_reporting,lastest_date_BQ=newest_month_BG,jobId=yt_entertainment[str(table)],onBehalfOfContentOwner=content_owners[str(content_owner)])
                            if bool(_retrieve_reports_monthly) == True:
                                print("----- The newest_date_on_BQ: {0} but {1} on API ".format(newest_month_BG,max(_retrieve_reports_monthly.keys())))
                                print("---------- ",len(_retrieve_reports_monthly)," reports need to retrieve ----------")
                                for date_retreive,value_retrieve in _retrieve_reports_monthly.items():
                ...
                for table in yt_affiliate:
                    if "owner_ad_revenue_raw" not in str(table):
                        ...
                        # TODO: MONTHLY REPORTS
                            ...
                            newest_month_BG  = get_lastest_month_func(content_owner="check_2_projects",table=table)
                            print("--------------------------"+table+"--------------------------")
                            _retrieve_reports_monthly = retrieve_reports(youtube_reporting,lastest_date_BQ=newest_month_BG,jobId=yt_affiliate[str(table)],onBehalfOfContentOwner=content_owners[str(content_owner)])
                            if bool(_retrieve_reports_monthly) == True:
                                print("----- The newest_date_on_BQ: {0} but {1} on API ".format(newest_month_BG,max(_retrieve_reports_monthly.keys())))
                                print("---------- ",len(_retrieve_reports_monthly)," reports need to retrieve ----------")
                                for date_retreive,value_retrieve in _retrieve_reports_monthly.items():
                ...
                for table in yt_th_music:
                    if "owner_ad_revenue_raw" not in str(table):
                        ...
                        # TODO: MONTHLY REPORTS
                            ...
                            newest_month_BG  = get_lastest_month_func(content_owner="check_2_projects",table=table)
                            print("--------------------------"+table+"--------------------------")
                            _retrieve_reports_monthly = retrieve_reports(youtube_reporting,lastest_date_BQ=newest_month_BG,jobId=yt_th_music[str(table)],onBehalfOfContentOwner=content_owners[str(content_owner)])
                            if bool(_retrieve_reports_monthly) == True:
                                print("----- The newest_date_on_BQ: {0} but {1} on API ".format(newest_month_BG,max(_retrieve_reports_monthly.keys())))
                                print("---------- ",len(_retrieve_reports_monthly)," reports need to retrieve ----------")
                                for date_retreive,value_retrieve in _retrieve_reports_monthly.items():
                ...
                for table in yt_th_entertainment:
                    if "owner_ad_revenue_raw" not in str(table):
                        ...
                        # TODO: MONTHLY REPORTS
                            ...
                            newest_month_BG  = get_lastest_month_func(content_owner="check_2_projects",table=table)
                            print("--------------------------"+table+"--------------------------")
                            _retrieve_reports_monthly = retrieve_reports(youtube_reporting,lastest_date_BQ=newest_month_BG,jobId=yt_th_entertainment[str(table)],onBehalfOfContentOwner=content_owners[str(content_owner)])
                            if bool(_retrieve_reports_monthly) == True:
                                print("----- The newest_date_on_BQ: {0} but {1} on API ".format(newest_month_BG,max(_retrieve_reports_monthly.keys())))
                                print("---------- ",len(_retrieve_reports_monthly)," reports need to retrieve ----------")
                                for date_retreive,value_retrieve in _retrieve_reports_monthly.items():
                ...
                for table in yt_th_affiliate:
                    if "owner_ad_revenue_raw" not in str(table):
                        ...
                        if bool(_retrieve_reports) == True:
                            ...
                            for date_retreive,value_retrieve in _retrieve_reports.items():
                                ...
                                for _file in check_files(FOLDER_PATH):
                                    if "p_content_owner_basic_a3" in _file and _file == content_owner+"_"+table+"_"+str(date_retreive).split(" ")[0]+".txt":
                                        try:
                                            p_content_owner_basic_a3(_file,table,"check_2_projects")
                                            print("{0} Inserted successfully".format(table))
                                            os.remove(_file)
                                        ...
                                        try:
                                            p_content_owner_estimated_revenue_a1(_file,table,"check_2_projects")
                                            print("{0} Inserted successfully".format(table))
                                            os.remove(_file)
                                        except (TypeError) as e:
                                            pass
                                        ...
                                        try:
                                            p_content_owner_video_metadata_a2(_file,table,"check_2_projects",str(date_retreive).split(" ")[0].replace("-",""))
                                            print("{0} Inserted successfully".format(table))
                                            os.remove(_file)
                                        except (TypeError) as e:
                                            pass
                        ...
                        # TODO: MONTHLY REPORTS
                            ...
                            newest_month_BG  = get_lastest_month_func(content_owner="check_2_projects",table=table)
                            print("--------------------------"+table+"--------------------------")
                            _retrieve_reports_monthly = retrieve_reports(youtube_reporting,lastest_date_BQ=newest_month_BG,jobId=yt_th_affiliate[str(table)],onBehalfOfContentOwner=content_owners[str(content_owner)])
                            if bool(_retrieve_reports_monthly) == True:
                                print("----- The newest_date_on_BQ: {0} but {1} on API ".format(newest_month_BG,max(_retrieve_reports_monthly.keys())))
                                print("---------- ",len(_retrieve_reports_monthly)," reports need to retrieve ----------")
                                for date_retreive,value_retrieve in _retrieve_reports_monthly.items():
                                    ...
                                    for _file in check_files(FOLDER_PATH):
                                        if "p_content_owner_ad_revenue_raw_a1" in _file and _file == content_owner + "_" + table + "_" + \
                                                str(date_retreive).split(" ")[0] + ".txt":
                                            try:
                                                p_content_owner_ad_revenue_raw_a1(_file, table, "check_2_projects")
                                                print("{0} Inserted successfully".format(table))
                                                os.remove(_file)
                                            except (TypeError) as e:
                                                pass
